...
function App() {
  ...
  const serviceCategories = [
    { name: 'فنادق', icon: '🏨', color: 'bg-blue-500', route: '/hotels' },
    { name: 'مطاعم ومقاهي', icon: '🍽️', color: 'bg-orange-500', route: '/restaurants' },
    { name: 'بنوك وماكينات ATM', icon: '🏦', color: 'bg-green-500', route: '/banks' },
    { name: 'وسائل انتقال', icon: '🚌', color: 'bg-purple-500', route: '/transportation' },
    { name: 'مراكز تسوق', icon: '🛍️', color: 'bg-pink-500', route: '/shopping-centers' }
  ]
            ...
            {serviceCategories.map((serviceCategory, index) => {
              return (
                <SwiperSlide key={index}>
                  <div 
                    className="service-card p-6 rounded-2xl h-full cursor-pointer"
                    onClick={() => handleServiceClick(serviceCategory.route)}
                  >
            ...
            <div className="p-6">
              ...
              <div className="space-y-6">
                ...
                {selectedLandmark.history && (
                  <div>
                    <h3 className="text-xl font-bold mb-2 text-gray-800">التاريخ</h3>
                    <p className="text-gray-600 leading-relaxed">{selectedLandmark.history}</p>
                  </div>
                )}
                
                {selectedLandmark.architecture && (
                  <div>
                    <h3 className="text-xl font-bold mb-2 text-gray-800">الطراز المعماري</h3>
                    <p className="text-gray-600 leading-relaxed">{selectedLandmark.architecture}</p>
                  </div>
                )}
                
                {selectedLandmark.activities && (
                  <div>
                    <h3 className="text-xl font-bold mb-2 text-gray-800">الأنشطة المتاحة</h3>
                    <p className="text-gray-600 leading-relaxed">{selectedLandmark.activities}</p>
                  </div>
                )}
                
                {selectedLandmark.facilities && (
                  <div>
                    <h3 className="text-xl font-bold mb-2 text-gray-800">المرافق</h3>
                    <p className="text-gray-600 leading-relaxed">{selectedLandmark.facilities}</p>
                  </div>
                )}
                
                {selectedLandmark.collections && (
                  <div>
                    <h3 className="text-xl font-bold mb-2 text-gray-800">المجموعات</h3>
                    <p className="text-gray-600 leading-relaxed">{selectedLandmark.collections}</p>
                  </div>
                )}
                
                {selectedLandmark.importance && (
                  <div>
                    <h3 className="text-xl font-bold mb-2 text-gray-800">الأهمية</h3>
                    <p className="text-gray-600 leading-relaxed">{selectedLandmark.importance}</p>
                  </div>
                )}
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg">
                  {selectedLandmark.visiting_hours && (
                    <div className="flex items-center gap-2">
                      <Clock className="h-5 w-5 text-blue-500" />
                      <div>
                        <p className="font-semibold text-gray-800">مواعيد الزيارة</p>
                        <p className="text-gray-600">{selectedLandmark.visiting_hours}</p>
                      </div>
                    </div>
                  )}
                  
                  {selectedLandmark.ticket_price && (
                    <div className="flex items-center gap-2">
                      <span className="text-green-500 text-xl">💰</span>
                      <div>
                        <p className="font-semibold text-gray-800">سعر التذكرة</p>
                        <p className="text-gray-600">{selectedLandmark.ticket_price}</p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">{selectedLandmark.location}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="font-semibold text-gray-800">التقييم</p>
                      <p className="text-gray-600">{selectedLandmark.rating} ({selectedLandmark.reviews} تقييم)</p>
                    </div>
                  </div>
                </div>
              </div>
